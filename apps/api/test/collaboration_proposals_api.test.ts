...
describe('collaboration proposals API', () => {
  ...
  it('Mind prompt contains ordered history', async () => {
    ...
    expect(prompt).toContain('Negotiation History:')
    expect(prompt).toContain('hist_prompt #1 by hist_a: Prompt One')
    expect(prompt).toContain('hist_prompt #2 by hist_e: Prompt Two')
    expect(prompt).toContain('hist_prompt #3 by hist_a: Prompt Three')
